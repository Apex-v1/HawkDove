...
type Tab = 'leaderboard' | 'charts' | 'pairings' | 'vote' | 'newsbox'
        ...
        {availableTabs.map(t => (
          <button key={t} onClick={() => setTab(t)}
            style={{ padding:'9px 20px', fontSize:11, letterSpacing:'0.12em', textTransform:'uppercase',
              ...
              color: tab===t ? (t==='vote'?'#e8a020':t==='newsbox'?'var(--green)':'var(--text)') : 'var(--text-dim)',
              borderBottom: tab===t ? `2px solid ${t==='vote'?'#e8a020':t==='newsbox'?'var(--green)':'var(--dove)'}` : '2px solid transparent' }}>
            {t}
          </button>
        ))}
        <div style={{ flex:1 }} />
  ...
  {/* President banner */}
              ...
              {voteDone ? (
                ...
                <>
                  ...
                  <button className="btn" onClick={handleVote} disabled={voteSubmitting}
                    style={{ width:'100%', padding:12, fontSize:12, borderColor:'#e8a020', color:'#e8a020' }}>
      ...
      <div style={{ padding:16, background:'var(--bg-card)', border:'1px solid var(--border)', gridColumn:'1/-1' }}>
        ...
        <div style={{ display:'flex', gap:12, alignItems:'flex-end', height:100 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}>
            ...
            <div style={{ width:60, height:Math.max(Math.round((eliminated.length/Math.max(students.length,1))*70),4), background:'rgba(232,55,42,0.08)', border:'1px solid rgba(232,55,42,0.3)', transition:'height 0.6s' }} />
